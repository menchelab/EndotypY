...
def main():
    ...
    print("\n5. Removing commas from terms...")
    terms_before = filtered_df['Term'].str.contains(',', na=False).sum()
    filtered_df['Term'] = filtered_df['Term'].str.replace(',', '', regex=False)
    terms_after = filtered_df['Term'].str.contains(',', na=False).sum()

    if terms_before > 0:
        print(f"   Removed commas from {terms_before:,} term occurrences")
        print(f"   Terms with commas remaining: {terms_after:,}")
    else:
        print("   No commas found in terms")

    # Keep only unique terms and remove nodeid column
    print("\n6. Keeping only unique terms and removing nodeid column...")
    ...
    output_file = 'output/terms_filtered.tsv'
    print(f"\n8. Saving unique terms to {output_file}...")
